...
| Chemin | Contenu |
|---|---|
| `kffi/` | Module KMP `org.graphiks:kffi` (3 backends JVM/Android/Native), fixtures C `src/ffiTest`, `src/main/cpp` (CMake + libffi glue), `src/nativeInterop`, README, `benchmarks/results/` |
...
| `.github/workflows/kffi-publish-snapshots.yml` | **Copier** | déjà switché `push: [main]` (M2.1) ; `submodules: recursive` inutile dans le repo cible (aucun submodule) |
| `.github/workflows/kffi-benchmark-ci.yml` | **Copier tel quel** | 3 jobs : `:kffi-benchmark-jvm:jmh`, `:kffi-benchmark-native:compileKotlinMacosArm64`, `:kffi-benchmark-android:assembleDebugAndroidTest` |
...
| `hs_err_pid*.log`, `replay_pid*.log` | **Ignorer** | junk local non tracké (gitignoré via `*.log`) — présent dans `kffi/` et à la racine, ne part pas |
...
- **kextract — AUCUNE dépendance build/runtime dans les 5 modules.** `rg "kextract" kffi*` ne trouve que des **commentaires** dans le code (`kffi_upcall.c`, `kffi_engine.c`, `JvmDowncallEngine.kt`, `JvmUpcallEngine.kt`, `JvmDowncallEngineShapeTableTest.kt`, `TestUpcallDispatchers.kt`) et le README kffi. **La génération vit dans le host :** `wgpu4k-native/build.gradle.kts:311-531` (task `generateBindingsFromHeader`, `dependsOn(":kextract:createKextractImage")`, `kextractDistribution`…). kffi est le **runtime** des bindings générés, jamais le générateur → kextract reste au host, le repo cible n'en a pas besoin.
- **wgpu4k-native — AUCUNE référence** (build ni runtime) dans les 5 modules. Les occurrences `wgpu4k` se limitent au README kffi (contexte parent) et aux chemins absolus machine dans `kffi/benchmarks/results/*.json` (artefacts de mesure, sans impact build).
...
| `settings.gradle.kts` | **Adapter** | `rootProject.name = "kffi"` ; supprimer `includeBuild("Kadre")` et les 6 includes host ; garder pluginManagement (foojay 0.8.0) + repos google/mavenCentral ; n'inclure que les 5 modules |
...
| `.github/workflows/kffi-publish-snapshots.yml` | **Copier** | push main ; retirer `submodules: recursive` (pas de submodule cible) |
| `.github/workflows/kffi-benchmark-ci.yml` | **Copier tel quel** | |
...
Copie isolée (rsync, sans `build/`, `.gradle/`, `hs_err*`, `replay_pid*`, `benchmarks/results/`) des 5 modules + `buildSrc/` + `gradle/` + `gradlew` + `gradle.properties` dans
...
1. **kextract n'est référencé nulle part dans les 5 modules au build** — uniquement dans des commentaires source et le README. La génération (`generateBindingsFromHeader` dans `wgpu4k-native/build.gradle.kts:311-531`) reste intégralement au host. Le repo cible est **runtime-only** : c'est exactement ce que le split doit produire.
...
8. **Junk local :** des `hs_err_pid*.log`/`replay_pid*.log` traînent dans `kffi/` et la racine (non trackés, gitignorés `*.log`) — ne partent pas dans le split, à ignorer.
...
L'inventaire confirme le découpage du plan P5 : les 5 modules `kffi*` sont **auto-suffisants** (aucune dépendance build/runtime sur wgpu4k-native, kextract, Kadre ou demo), et le **test de faisabilité est vert** pour `:kffi:jvmTest`, SPI, JMH, native (cinterop + harness) et android (assemble + staging). Le repo cible doit copier la machinerie racine (wrapper, version catalog, gradle.properties, .gitignore, LICENSE, buildSrc) et **adapter** `settings.gradle.kts`, le root `build.gradle.kts` (task `cleanPublicationVerificationRepository` obligatoire, vérification wgpu retirée) et les workflows (2 copiés, 1 extrait). Le split M4 peut s'exécuter sur cette base.
