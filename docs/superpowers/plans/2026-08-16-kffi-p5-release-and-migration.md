...
- **F10 (tracké, post-P5)** : wiring strict-validation incomplet dans wgpu4k-native/build.gradle.kts (M4.3) — le fix lazy `withType` couvre les compiles (compileDebugKotlinAndroid, compileKotlin*) mais pas `KotlinCompileCommon` (metadata), sourcesJar/dokka. `verifyPublicationMetadata` sans `-x :wgpu4k-native:generateBindingsFromHeader` reste rouge à froid (F3 non fermé). Compléter le wiring (withType<KotlinCompileCommon> + sourcesJar + dokka) en follow-up CI.
- **F11 (séquençage M4.4, documenté)** : la CI hôte résout `org.graphiks:kffi:1.0.0-SNAPSHOT` depuis Sonatype — vide tant que le repo cible n'a pas publié (workflow kffi-publish-snapshots après fusion de la PR M4.4 + credentials org). Séquençage obligatoire : fusion PR → snapshots publiés → CI hôte verte. En local, mavenLocal prime (doc commentaire settings).
